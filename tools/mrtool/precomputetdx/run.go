...
const (
	...
	defaultOvmfVersion = "edk2-stable202502"
	defaultQemuVersion = "11.0.0" // or higher
	defaultRamSize     = "4G"
)
...
func getConfig(cmd *cli.Command) (*Config, error) {
	...
	if cmd.IsSet(ovmfversionFlag) {
		...
		c.OvmfVersion = defaultOvmfVersion
	}
	if cmd.IsSet(qemuversionFlag) {
		c.QemuVersion = cmd.String(qemuversionFlag)
	} else {
		c.QemuVersion = defaultQemuVersion
	}

	ramStr := defaultRamSize
...
func (c *Config) print() {
	...
	if c.RamSize != 0 {
		log.Debugf("\tRAM size: %#x bytes", c.RamSize)
	}
