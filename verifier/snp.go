...
func getSnpTcb(codeName string, tcb uint64) ar.SnpTcb {
...
func verifySnpTcb(codeName string, s snpreport, v ar.SnpTcb) (ar.TcbCheck, bool) {
	...
	curr := getSnpTcb(codeName, s.CurrentTcb)
	comm := getSnpTcb(codeName, s.CommittedTcb)
	laun := getSnpTcb(codeName, s.LaunchTcb)
	repo := getSnpTcb(codeName, s.ReportedTcb)
	...
	log.Tracef("Using minimum report TCB: BL")
...
func verifySnpExtensions(codeName string, cert *x509.Certificate, report *snpreport) ([]ar.Result, bool) {
	...
	tcb := getSnpTcb(codeName, report.ReportedTcb)
