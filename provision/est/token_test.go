...
// TestVerifyToken_ConcurrentSingleUse fires N goroutines against a MaxUses=1
// token simultaneously and asserts exactly one succeeds.
...
// TestVerifyToken_BackwardCompat verifies that an old-format token file
// (no MaxUses/Uses fields) is still accepted as unlimited.
func TestVerifyToken_BackwardCompat(t *testing.T) {
	dir := t.TempDir()

	// Create a token using the old API (no MaxUses → 0 → unlimited).
	token, err := CreateToken(dir, TokenOptions{})
	if err != nil {
		t.Fatalf("CreateToken: %v", err)
	}
...
func TestCreateToken_Options(t *testing.T) {
	...
	// Mismatched subject: should fail but preserve the token.
	if err := VerifyToken(dir, token, "other"); err == nil {
		t.Error("wrong subject should be rejected")
	}

	// Correct subject: 5 uses allowed.
	for i := range 5 {
	...
	// Cleanup: remove temp dir entries.
	_ = os.RemoveAll(dir)
}
