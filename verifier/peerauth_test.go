...
// buildMeta constructs a MetadataSummary whose image description carries the
// given allowed peers. A nil slice models "no Peers block" (opt-out).
...
// No AllowedPeers on the image description → not opted in → pass.
...
// Empty AllowedPeers is equivalent to no policy.
...
// A peer that declared a Peers policy expects an identifiable counterparty.
// A caller with no local role cannot satisfy that intent → fail-closed.
...
// No policy AND no local role: pass; there is nothing to check.
...
// nil metadata is a caller programming error, not a policy state → fail-closed.
