...
The `cmcd` itself uses two key-pairs and corresponding certificates:

The Attestation Key (AK) key-pair is used to sign the hardware attestation reports (e.g. TPM or
Intel TDX Quote). Depending on the used technology, the private key usually resides in hardware and
the certificate chain is provided by the manufacturer, e.g. the Intel TDX PCK key and certificate
chain or the AMD SEV-SNP VCEK key and certificate chain. The

The Identity Key (IK) key-pair is generated by the `cmcd` and used to sign the overall attestation
reports and to establish attested TLS or HTTPS connections. The certificate chain is created by the
`estserver`. The key can also reside in hardware, e.g., in a TPM or vTPM.
...
### azuredriver
The azuredriver is used for Intel TDX and AMD SEV-SNP CVMs on Microsoft Azure. Azure employs a vTPM
within the CVM for runtime measurements. To obtain hardware reports (TDX or SNP), a nonce (user
