...
// the reference values and the compatibility of software artefacts. It also enforces
// peer authorization from signed metadata using the provided localRole.
func Verify(
	arRaw, nonce []byte,
...
Loop:
	...
	// Peer authorization: check if we are allowed to establish a connection with this peer in case
	// peer policies are provided. Skipped in bootstrap contexts (see VerifyBootstrap).
	if !skipPeerAuth {
