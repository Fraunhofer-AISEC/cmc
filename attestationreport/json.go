...
func (s JsonSerializer) Sign(data []byte, driver Driver, sel KeySelection) ([]byte, error) {
	...
	joseSigner, err = jose.NewSigner(
		...
		opt.WithHeader("x5c", certsb64).WithHeader("jwk", pub),
	)
	if err != nil {
...
func (s JsonSerializer) Verify(data []byte, roots []*x509.Certificate, useSystemCerts bool) (MetadataResult, []byte, bool) {
	...
	for i, sig := range jwsData.Signatures {
		...
		index[i], _, payloads[i], err = jwsData.VerifyMulti(sig.Protected.JSONWebKey)
		if err == nil {
