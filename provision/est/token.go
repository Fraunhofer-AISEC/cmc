...
type TokenData struct {
	...
	// MaxUses is the maximum number of successful retrievals. 0 means unlimited.
	MaxUses int `json:"maxUses,omitempty"`
	// Uses is the number of successful retrievals so far.
	Uses int `json:"uses,omitempty"`
	// Subject, if non-empty, restricts the token to callers that present the subject.
	Subject string `json:"subject,omitempty"`
}

// TokenOptions configures a token created by CreateToken.
type TokenOptions struct {
	// MaxUses is the maximum number of successful redemptions. 0 means unlimited.
	MaxUses int
	// Subject, if non-empty, restricts the token to callers presenting this subject.
	Subject string
}
...
// subject is compared against the token's Subject field when that field is non-empty.
func VerifyToken(dir string, presentedToken string, subject string) error {
	...
	// We exclusively own claimedPath.
	...
	// For unlimited tokens restore the file unchanged.
	if td.MaxUses == 0 {
		if err := os.Rename(claimedPath, fullpath); err != nil {
	...
	// Limited-use: record this redemption.
	td.Uses++
	...
	// Still has remaining uses: write updated state back atomically.
...
func pruneExpiredTokenFiles(dir string) error {
	...
	for _, e := range entries {
		...
		name := e.Name()
		// Remove stale claimed/tmp files left by a crashed process (>1 min old).
