...
func (s JsonSerializer) Verify(data []byte, verifier interface{}) (MetadataResult, []byte, bool) {
	...
	case VerifierOptions:
		switch verifier {
		case VerifyWithSystemCerts:
		...
		case VerifyWithSelfSignedCert:
			log.Debug("Using embedded self-signed cert for JWS verification")
			cert, err := getSelfSignedJwsCert(data)
			if err != nil {
				log.Warnf("Failed to get self signed cert: %v", err)
				result.Summary.SetErr(JWSExtractSelfSignedCert)
				return result, nil, false
			}
			rootpool = x509.NewCertPool()
			rootpool.AddCert(cert)
		default:
			log.Warnf("Unknown verifier options %v", verifier)
			result.Summary.SetErr(JWSNoKeyOrCert)
			return result, nil, false
		}
