...
func verifyAzureMeasurements(measurements []ar.Measurement, nonce []byte, rootManifest *ar.MetadataResult,
	...
	// Verify Azure Chain of Trust: vTPM AKpublic must match CC report nonce
	err = VerifyAzureCoT(ccMeasurement.Claims, hwreportNonce, vtpmAkCert)
	if err != nil {
		r := ar.MeasurementResult{
			Type: ccMeasurement.Type,
		}
	...
	// Verify TDX measurements with fixed nonce
	...
	// Verify SNP measurement with fixed nonce
...
func VerifyAzureCoT(rtdataRaw, reportNonce []byte, vtpmAkCert *x509.Certificate) error {
