...
func New(ak any, ik any, vcekCache, ekDbPath string, verifyEkCert bool) (*Client, error) {
	return &Client{
		ak: ak,
		ik: ik,
		snpConf: &provision.SnpConfig{
...
func (c *Client) CaCerts(addr string) ([]*x509.Certificate, error) {
	...
	// TODO determine if needed, error handling
	return nil, nil
}
...
func (c *Client) SimpleEnroll(csr *x509.CertificateRequest) (*x509.Certificate, error) {
	...
	if strings.Contains(csr.Subject.CommonName, "ak") {
		key = c.ak.(crypto.PublicKey)
	} else if strings.Contains(csr.Subject.CommonName, "ik") {
		key = c.ik.(crypto.PublicKey)
	} else {
		return nil, fmt.Errorf("failed to determine key type by common name %q", csr.Subject.CommonName)
	}
	...
	certDer, err := x509.CreateCertificate(rand.Reader, tmpl, tmpl, csr.PublicKey, key)
	if err != nil {
...
func (c *Client) TpmActivateEnroll(
	...
	certDer, err := x509.CreateCertificate(rand.Reader, tmpl, tmpl, csr.PublicKey, c.ik)
	if err != nil {
...
func (c *Client) GetSnpVcek(addr string, chipId [64]byte, tcb uint64) (*x509.Certificate, error) {
